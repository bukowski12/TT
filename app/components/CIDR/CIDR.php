...
class CIDR {
	...
	public static function cidrToRange($cidr) {
	 ...
	 /**
	 * method cidrToList.
	 * Returns an array of IPv4 addresses from CIDR
         * address as the first entry. If you need to check to see if an IPv4
         * address is within range please use the IPisWithinCIDR method above.
	 * Usage:
	 *     CIDR::cidrToList("127.0.0.128/255.255.255.0");
	 * Result:
         *     array(2) {
         *       [0]=> string(11) "127.0.0.129"
         *       [1]=> string(11) "127.0.0.130"
         *			.....
         *     }
	 * @param $cidr string CIDR block
	 * @return Array list .
	...
	public static function cidrToList($cidr) {
		$list = array();
		$ip_arr = explode('/', $cidr);
		$ip_arr[1] = self::maskToCIDR($ip_arr[1]);
		$ip_count = 1 << (32 - $ip_arr[1]);
		$start = ip2long($ip_arr[0]);
		for ($i = 0; $i < $ip_count; $i++) {

			$list[] = long2ip($start + $i);
...
/*
    	...
    	$nw = ($ip & $nm);
    	$bc = $nw | (~$nm);
	//printf ('%u  %u  %u  %u  ', $nm, $nw, $bc, $ip);exit();
	//for($zm=1;($nw + $zm)<=($bc - 1);$zm++)
    	for($zm=1;$zm<=($bc - 1);$zm++)
        {
        	$list[$zm-1] =  long2ip($nw + $zm);
        	dump ($list);exit();
        }
        //dump ($list);exit();*/
		return $list;
